...
public class PMview extends View {
    ...
    public PMview(final Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawCenterText(Canvas canvas) {
        ...
        String content = "";
        if(currentNum <=maxNum*1/10){
            content += text[0];
        }else if(currentNum > maxNum*1/10 && currentNum <= maxNum*1/5){
            content += text[1];
    ...
    private void drawScale(Canvas canvas) {
        ...
        for (int i = 0; i <= 30; i++) {
            ...
            if(i%6 == 0){   //画粗刻度和刻度值
                ...
                paint.setStrokeWidth(dp2px(2));
                paint.setAlpha(0x50);
                drawText(canvas,text[(i-2)/2],paint);
            }
...
//            }
            if( i==9 || i==15 || i==21 || i==27){  //画刻度区间文字
                paint.setStrokeWidth(dp2px(2));
                paint.setAlpha(0x50);
                drawText(canvas,text[(i-3)/6+1], paint);
            }
